...
public class ItemExtendedNetherPickaxe extends ItemPickaxe
{
    public ItemExtendedNetherPickaxe(int par1, EnumToolMaterial par2EnumToolMaterial)
    {
	...
	public boolean hitEntity(ItemStack par1ItemStack, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase) {
		if (PluginChecks.getNetherFusionInstalled() && this.itemID == SimpleOresEW.extendeddragonbezoarPick.itemID) {
		...
		if (PluginChecks.getNetherFusionInstalled() && this.itemID == SimpleOresEW.extendedpyralisPick.itemID) {
			par1ItemStack.damageItem(1, par3EntityLivingBase);
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
		if (PluginChecks.getNetherFusionInstalled() && (this.itemID == SimpleOresEW.extendeddragonbezoarPick.itemID || this.itemID == SimpleOresEW.extendedpyralisPick.itemID)) {
			if (par7 == 0) {
				par5--;
			}
			if (par7 == 1) {
...
public boolean onBlockStartBreak(ItemStack itemstack, int i, int j, int k, EntityPlayer player) {
		...
		if (((PluginChecks.getNetherInstalled() && itemID == SimpleOresEW.extendedfyritePick.itemID) || (PluginChecks.getNetherFusionInstalled() && itemID == SimpleOresEW.extendeddragonbezoarPick.itemID)) && !player.capabilities.isCreativeMode) {
			Boolean flag = false;
